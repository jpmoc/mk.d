...
# KCL_CLUSTER_STERN?= stern
KCL_CLUSTER_TAILCOLOR_MODE?= auto
KCL_CLUSTER_TAILCONTAINERS_STATE?= running
# KCL_CLUSTER_TAILEXCLUDECONTAINERS_REGEX?= "istio-|fluentd-.*"
# KCL_CLUSTER_TAILEXCLUDELOGLINES_REGEX?= "^POST|HealthChecker"
KCL_CLUSTER_TAILHISTORY_COUNT?= 0# -1
KCL_CLUSTER_TAILOUTPUT_MODE?= default
KCL_CLUSTER_TAILPODS_REGEX?= '.*'#
# KCL_CLUSTER_TAILSINCE_DURATION?= 2m
...
KCL_CLUSTER_STERN?= $(KCL_STERN)

# Option parameters
__KCL_COLOR__CLUSTER= $(if $(KCL_CLUSTER_TAILCOLOR_MODE),--color $(KCL_CLUSTER_TAILCOLOR_MODE))
__KCL_CONTAINER_STATE__CLUSTER= $(if $(KCL_CLUSTER_TAILCONTAINERS_STATE),--container-state $(KCL_CLUSTER_TAILCONTAINERS_STATE))
__KCL_EXCLUDE__CLUSTER= $(if $(KCL_CLUSTER_TAILEXCLUDELOGLINES_REGEX),--exclude $(KCL_CLUSTER_TAILEXCLUDELOGLINES_REGEX))
__KCL_EXCLUDE_CONTAINER__CLUSTER= $(if $(KCL_CLUSTER_TAILEXCLUDECONTAINERS_REGEX),--exclude-container $(KCL_CLUSTER_TAILEXCLUDECONTAINERS_REGEX))
__KCL_OUTPUT__CLUSTER= $(if $(KCL_CLUSTER_TAILOUTPUT_MODE),--output $(KCL_CLUSTER_TAILOUTPUT_MODE))
__KCL_SINCE__CLUSTER= $(if $(KCL_CLUSTER_TAILSINCE_DURATION),--since $(KCL_CLUSTER_TAILSINCE_DURATION))
__KCL_TAIL__CLUSTER= $(if $(KCL_CLUSTER_TAILHISTORY_COUNT),--tail $(KCL_CLUSTER_TAILHISTORY_COUNT))
...
_kcl_view_framework_parameters ::
	...
	@echo '    KCL_CLUSTER_TAILCOLOR_MODE=$(KCL_CLUSTER_TAILCOLOR_MODE)'
	@echo '    KCL_CLUSTER_TAILCONTAINERS_STATE=$(KCL_CLUSTER_TAILCONTAINERS_STATE)'
	@echo '    KCL_CLUSTER_TAILHISTORY_COUNT=$(KCL_CLUSTER_TAILHISTORY_COUNT)'
	@echo '    KCL_CLUSTER_TAILEXCLUDECONTAINERS_REGEX=$(KCL_CLUSTER_TAILEXCLUDECONTAINERS_REGEX)'
	@echo '    KCL_CLUSTER_TAILEXCLUDELOGLINES_REGEX=$(KCL_CLUSTER_TAILEXCLUDELOGLINES_REGEX)'
	@echo '    KCL_CLUSTER_TAILOUTPUT_MODE=$(KCL_CLUSTER_TAILOUTPUT_MODE)'
	@echo '    KCL_CLUSTER_TAILPODS_REGEX=$(KCL_CLUSTER_TAILPODS_REGEX)'
	@echo '    KCL_CLUSTER_TAILSINCE_DURATION=$(KCL_CLUSTER_TAILSINCE_DURATION)'
...
_kcl_tail_cluster:
	@$(INFO) '$(KCL_UI_LABEL)Tailing pods in cluster "$(KCL_CLUSTER_NAME)" ...'; $(NORMAL)
	$(_KCL_TAIL_CLUSTER_|)$(KCL_CLUSTER_STERN) --all-namespaces $(__KCL_COLOR__CLUSTER) $(__KCL_CONTAINER_STATE__CLUSTER) $(__KCL_EXCLUDE__CLUSTER) $(__KCL_EXCLUDE_CONTAINER__CLUSTER) $(__KCL_OUTPUT__CLUSTER) $(__KCL_SINCE__CLUSTER) $(__KCL_TAIL__CLUSTER) $(KCL_CLUSTER_TAILPODS_REGEX) $(|_KCL_TAIL_CLUSTER)
