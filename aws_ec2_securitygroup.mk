...
# UI parameters
EC2_UI_SHOW_SECURITYGROUP_EGRESSES_FIELDS?= .{FromPort:FromPort,ipProtocol:IpProtocol,ToPort:ToPort,ipRanges:IpRanges[*].CidrIp|join('  ',@),description:IpRanges[0].Description} 
EC2_UI_SHOW_SECURITYGROUP_INGRESSES_FIELDS?= .{FromPort:FromPort,ipProtocol:IpProtocol,ToPort:ToPort,ipRanges:IpRanges[*].CidrIp|join('  ',@),description:IpRanges[0].Description}
EC2_UI_VIEW_SECURITYGROUPS_FIELDS?= .{GroupId:GroupId,GroupName:GroupName,description:Description,vpcId:VpcId}
EC2_UI_VIEW_SECURITYGROUPS_SET_FIELDS?= $(EC2_UI_VIEW_SECURITYGROUPS_FIELDS)
EC2_UI_VIEW_SECURITYGROUPS_SET_QUERYFILTER?=
...
_ec2_view_framework_targets ::
	...
	@echo '    _ec2_view_securitygroups                   - View all security-groups'
	@echo '    _ec2_view_securitygroups_set               - View a set of security-groups'
...
_ec2_show_securitygroup: _ec2_show_securitygroup_egresses _ec2_show_securitygroup_ingresses _ec2_show_securitygroup_description
...
_ec2_show_securitygroup_egresses:
	...
	$(AWS) ec2 describe-security-groups $(__EC2_GROUP_IDS__SECURITYGROUP) --query "SecurityGroups[].IpPermissionsEgress[]$(EC2_UI_SHOW_SECURITYGROUP_EGRESSES_FIELDS)"
...
_ec2_show_securitygroup_ingresses:
	...
	$(AWS) ec2 describe-security-groups $(__EC2_GROUP_IDS__SECURITYGROUP) --query "SecurityGroups[].IpPermissions[]$(EC2_UI_SHOW_SECURITYGROUP_INGRESSES_FIELDS)"

_ec2_view_securitygroups:
	@$(INFO) '$(EC2_UI_LABEL)Viewing ALL security-groups ...'; $(NORMAL)
	$(AWS) ec2 describe-security-groups $(_X__EC2_FILTERS__SECURITYGROUP) --query "SecurityGroups[]$(EC2_UI_VIEW_SECURITYGROUPS_FIELDS) | sort_by(@,&GroupName)" 

_ec2_view_securitygroups_set:
	@$(INFO) '$(EC2_UI_LABEL)Viewing security-groups-set "$(EC2_SECURITYGROUPS_SET_NAME)" ...'; $(NORMAL)
	@$(WARN) 'Security-groups are grouped based on the provided filter and optional query-filter'; $(NORMAL)
	$(AWS) ec2 describe-security-groups $(__EC2_FILTERS__SECURITYGROUPS) --query "SecurityGroups[$(EC2_UI_VIEW_SECURITYGROUPS_SET_QUERYFILTER)]$(EC2_UI_VIEW_SECURITYGROUPS_SET_FIELDS) | sort_by(@,&GroupName)" 
