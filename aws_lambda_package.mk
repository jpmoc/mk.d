...
LBA_PACKAGE_FILEPATH?= $(LBA_PACKAGES_DIRPATH)$(LBA_PACKAGE_FILENAME)
...
_lba_list_targets ::
	...
	@echo '    _lba_list_packages               - List all deployment-packages'
	@echo '    _lba_list_packages_set           - List a set of deployment-packages'
...
_lba_create_package:
	@$(INFO) '$(LBA_UI_LABEL)Creating deployment-package "$(LBA_PACKAGE_NAME)" ...'; $(NORMAL)
	mkdir -p $(LBA_PACKAGES_DIRPATH)/
	cd $(LBA_PACKAGE_ZIP_EXECPATH); $(ZIP) $(LBA_PACKAGE_FILEPATH) $(LBA_PACKAGE_ZIP_CONTENT)

_lba_delete_package:
	@$(INFO) '$(LBA_UI_LABEL)Deleting deployment-package "$(LBA_PACKAGE_NAME)" ...'; $(NORMAL)
	-rm -rf $(LBA_PACKAGE_FILEPATH)

_lba_list_packages:
	@$(INFO) '$(LBA_UI_LABEL)Listing all deployment-packages ...'; $(NORMAL)
	-ls -la $(LBA_PACKAGES_DIRPATH)/

_lba_list_packages_set:

_LBA_SHOW_PACKAGE_TARGETS?= _lba_show_package_description
_lba_show_package: $(_LBA_SHOW_PACKAGE_TARGETS)

_lba_show_package_description:
	@$(INFO) '$(LBA_UI_LABEL)Showing description of deployment-package "$(LBA_PACKAGE_NAME)" ...'; $(NORMAL)
