...
# Customizations
_CFN_LIST_CHANGESETS_FIELDS?= .{ChangeSetName:ChangeSetName,StackName:StackName,_Description:Description,_Status:Status,_ExecutionStatus:ExecutionStatus}
...
_cfn_list_targets ::
	...
	@echo '    _cfn_list_changesets              - List all change-sets'
...
_cfn_create_changeset:
	@$(INFO) '$(CFN_UI_LABEL)Creating change-set "$(CFN_CHANGESET_NAME)" ...'; $(NORMAL)
...
_cfn_delete_changeset:
	@$(INFO) '$(CFN_UI_LABEL)Deleting change-set "$(CFN_CHANGESET_NAME)" ...'; $(NORMAL)
	$(AWS) cloudformation delete-change-set $(__CFN_CHANGE_SET_NAME) $(__CFN_STACK_NAME)

_cfn_execute_changeset:
	@$(INFO) '$(CFN_UI_LABEL)Executing change-set "$(CFN_CHANGESET_NAME)" ...'; $(NORMAL)
	$(AWS) cloudformation delete-change-set $(__CFN_CHANGE_SET_NAME) $(__CFN_STACK_NAME)

_cfn_list_changesets:
	@$(INFO) '$(CFN_UI_LABEL)Listing change-sets of stack "$(CFN_STACK_NAME)" ...'; $(NORMAL)
	$(AWS) cloudformation list-change-sets $(__CFN_STACK_NAME) --query "Summaries[]$(_CFN_LIST_CHANGESETS_FIELDS)"

_cfn_show_changeset:
	@$(INFO) '$(CFN_UI_LABEL)Showing change-set "$(CFN_CHANGESET_NAME)" ...'; $(NORMAL)
